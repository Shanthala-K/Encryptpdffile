...
Public Class frmencrypt
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Dim totalMinutes As Integer = Convert.ToInt32(txtBxTotalMin.Text)
        Dim strtemp As String
        Dim ts As TimeSpan = TimeSpan.FromMinutes(totalMinutes)
        If ts.Hours = 0 Then
            strtemp = ts.Minutes & " Minutes"
        Else
            strtemp = ts.Hours & " hrs " & ts.Minutes & "Mins"
        End If
        ' Dim strtemp As String = String.Format("{0} : {1}", ts.Hours, ts.Minutes)
        MsgBox(strtemp)
    End Sub
End Class
